...
export class Scheduler {
  ...
  private async executeTask(task: DaemonTask): Promise<void> {
    ...
    try {
      ...
      // Detect common LLM connection failures and provide helpful messages
      const isOllamaError = errMsg.includes('ECONNREFUSED') || errMsg.includes('fetch failed') || errMsg.includes('localhost:11434');
      const isApiKeyError = errMsg.includes('API key') || errMsg.includes('401') || errMsg.includes('Unauthorized');

      if (isOllamaError) {
        this.ctx.log.error(
          `Task ${task.name} failed: Cannot connect to Ollama. ` +
          `Either install & start Ollama, or change llm.mode to "api" in Configuration.`
        );
      } else if (isApiKeyError) {
        this.ctx.log.error(
          `Task ${task.name} failed: Invalid or missing API key. ` +
          `Set a valid llm.apiKey in Configuration.`
        );
      } else {
        this.ctx.log.error(`Task ${task.name} failed: ${errMsg}`);
      }
    } finally {
  ...
  getStatus(): {
    ...
    return {
      ...
      tasks: this.tasks.map(t => {
        ...
        const configKey = this.taskConfigKey(t.name);
        const enabled = this.ctx.config.getBool(`task.${configKey}.enabled`);
        const intervalMinutes = this.ctx.config.getNumber(`task.${configKey}.interval`);
        return {
          name: t.name,
