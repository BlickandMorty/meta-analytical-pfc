...
function initDb() {
  ...
  // Add 'attachments' column to message table if missing (schema added it but
  // CREATE TABLE IF NOT EXISTS doesn't alter existing tables).
  try {
    sqlite.prepare(`SELECT attachments FROM message LIMIT 0`).run();
  } catch {
    sqlite.prepare(`ALTER TABLE message ADD COLUMN attachments TEXT`).run();
  }
