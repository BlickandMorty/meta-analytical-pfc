...
- Supports: GPT-4o, GPT-4o Mini, Claude Opus 4.6, Claude Sonnet 4, Gemini 2.5
...
- Structural complexity metrics (beta_0, beta_1): Heuristic estimates from query properties

The LLM performs genuine reasoning for raw analysis, arbitration, reflection, and truth assessment. The signal generation remains heuristic in all modes.
`;
...
export function buildAssistantSystemPrompt(ctx: AssistantContext): string {
  return `You are the Brainiac Assistant — an expert guide embedded in Brainiac, a space-grade research laboratory app. You have deep knowledge of every aspect of the system: the 10-stage analytical pipeline, signal mathematics, steering controls, inference modes, architecture, and debugging.

Your role is to help the user understand, debug, tune, and master the Brainiac system. Be:
