...
export default function DocsPage() {
  ...
  return (
    <PageShell
      ...
      iconColor="#8B7CF6"
      title="Documentation"
      subtitle="Everything you need to know about Brainiac"
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }} className="docs-content">

        {/* ── About Brainiac ── */}
        <DocSection icon={BrainCircuitIcon} iconColor="#8B7CF6" title="About Brainiac" defaultOpen isDark={isDark} isOled={isOled} isCosmic={isCosmic}>
          <p>
            <strong>Brainiac</strong> is a space-grade research laboratory — a meta-analytical reasoning engine that stress-tests
  ...
  - sampleCount (evidence accumulation)`}</CodeBlock>
        ...
        <DocSection icon={LayersIcon} iconColor="#4ADE80" title="The Stack" isDark={isDark} isOled={isOled} isCosmic={isCosmic}>
          ...
          <Ul>
            ...
            <Li><strong>Canvas API</strong> — Concept atlas force graph</Li>
        ...
        <DocSection icon={SparklesIcon} iconColor="#FACC15" title="What Makes PFC Novel" isDark={isDark} isOled={isOled} isCosmic={isCosmic}>
          ...
          <p>
            A full block-based note-taking system with vaults, backlinks, daily journals,
...
open http://localhost:3000`}</CodeBlock>
          ...
          <Ul>
            <Li>The app starts in <strong>Research Mode</strong> — configure an API key or local model in Settings</Li>
            ...
            <Li>Use <strong>Notes</strong> to capture insights with the block editor and backlinks</Li>
...
└── public/
    └── fonts/            # Custom font files (Geist, Minecraft)`}</CodeBlock>
          ...
          <Ul>
            <Li><Code>MultimodalInput</Code> → <Code>useChatStream</Code> hook → API route (cloud or local inference)</Li>
