...
function GlassInput({
  ...
  const shared: React.CSSProperties = {
    ...
    color: isDark ? 'rgba(237,224,212,1)' : 'rgba(28,27,31,0.85)',
...
function ScoreRing({ score, max, label, isDark, isDefaultLight, size = 48 }: {
  score: number; max: number; label: string; isDark: boolean; isDefaultLight?: boolean; size?: number;
}) {
  const pct = Math.min(score / max, 1);
  const r = (size - 6) / 2;
  const circumference = 2 * Math.PI * r;
  const offset = circumference * (1 - pct);
  const color = isDefaultLight ? '#9E9E9E' : pct >= 0.75 ? '#34D399' : pct >= 0.5 ? '#FBBF24' : pct >= 0.25 ? '#FB923C' : '#F87171';
...
function PaperCard({ paper, isDark, isDefaultLight, onSave }: {
  paper: S2Paper; isDark: boolean; isDefaultLight?: boolean; onSave: (paper: S2Paper) => void;
}) {
        ...
        <div style={{ display: 'flex', gap: '0.25rem', flexShrink: 0 }}>
          ...
          <button
            ...
            style={{
              ...
              color: saved ? (isDefaultLight ? '#9E9E9E' : '#34D399') : (isDark ? 'rgba(156,143,128,0.5)' : 'rgba(0,0,0,0.3)'),
            }}
          >
...
function PaperSearchTab({ isDark, isDefaultLight }: { isDark: boolean; isDefaultLight?: boolean }) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      ...
      {error && (
        <div style={{
          ...
          background: isDefaultLight ? '#F5F5F5' : isDark ? 'rgba(248,113,113,0.08)' : 'rgba(248,113,113,0.06)',
          color: isDefaultLight ? '#9E9E9E' : '#F87171', fontSize: '0.75rem',
        }}>
          {error}
        </div>
      )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
        {results.map((paper) => (
          <PaperCard key={paper.paperId} paper={paper} isDark={isDark} isDefaultLight={isDefaultLight} onSave={handleSave} />
        ))}
      </div>
    </div>
  );
...
function NoveltyCheckTab({ isDark, isDefaultLight }: { isDark: boolean; isDefaultLight?: boolean }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      {!inferenceConfig && (
        <div style={{
          padding: '0.75rem 1rem', borderRadius: '0.75rem',
          background: isDefaultLight ? '#F5F5F5' : isDark ? 'rgba(251,191,36,0.08)' : 'rgba(251,191,36,0.06)',
          color: isDefaultLight ? '#9E9E9E' : '#FBBF24', fontSize: '0.75rem',
        }}>
      ...
      {result && (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={SPRING_SOFT}>
          ...
          <div style={{
            ...
            background: isDefaultLight ? '#F5F5F5'
              : isNovel
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
              <div style={{
                ...
                color: isDefaultLight ? '#9E9E9E' : isNovel ? '#34D399' : '#F87171',
              }}>
...
function PaperReviewTab({ isDark, isDefaultLight }: { isDark: boolean; isDefaultLight?: boolean }) {
  const inferenceConfig = useInferenceConfig();
  const [title, setTitle] = useState('');
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={SPRING_SOFT}
          ...
          <div style={{
            ...
            <span style={{
              ...
              color: isDefaultLight ? '#9E9E9E' : isAccept ? '#34D399' : '#F87171',
            }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', justifyContent: 'center' }}>
            <ScoreRing score={scores.overall ?? 0} max={10} label="Overall" isDark={isDark} isDefaultLight={isDefaultLight} size={56} />
            <ScoreRing score={scores.originality ?? 0} max={4} label="Originality" isDark={isDark} isDefaultLight={isDefaultLight} />
            <ScoreRing score={scores.quality ?? 0} max={4} label="Quality" isDark={isDark} isDefaultLight={isDefaultLight} />
            <ScoreRing score={scores.clarity ?? 0} max={4} label="Clarity" isDark={isDark} isDefaultLight={isDefaultLight} />
            <ScoreRing score={scores.significance ?? 0} max={4} label="Significance" isDark={isDark} isDefaultLight={isDefaultLight} />
            <ScoreRing score={scores.soundness ?? 0} max={4} label="Soundness" isDark={isDark} isDefaultLight={isDefaultLight} />
            <ScoreRing score={scores.presentation ?? 0} max={4} label="Presentation" isDark={isDark} isDefaultLight={isDefaultLight} />
            <ScoreRing score={scores.contribution ?? 0} max={4} label="Contribution" isDark={isDark} isDefaultLight={isDefaultLight} />
            <ScoreRing score={scores.confidence ?? 0} max={5} label="Confidence" isDark={isDark} isDefaultLight={isDefaultLight} />
          </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
                    {strengths.map((s, i) => (
                      <div key={i} style={{
                        ...
                        <span style={{ color: isDefaultLight ? '#BDBDBD' : '#34D399', fontWeight: 700, flexShrink: 0 }}>+</span>
                        {s}
                      </div>
                    ))}
                  </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
                    {weaknesses.map((w, i) => (
                      <div key={i} style={{
                        ...
                        <span style={{ color: isDefaultLight ? '#BDBDBD' : '#F87171', fontWeight: 700, flexShrink: 0 }}>-</span>
                        {w}
                      </div>
                    ))}
                  </div>
...
function CitationSearchTab({ isDark, isDefaultLight }: { isDark: boolean; isDefaultLight?: boolean }) {
  const inferenceConfig = useInferenceConfig();
      ...
      {result && (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={SPRING_SOFT}
          ...
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            ...
            {copied && (
              <span style={{ fontSize: '0.6875rem', color: isDefaultLight ? '#9E9E9E' : '#34D399', display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '0.75rem' }}>
                        ...
                        {relevanceScore != null && (
                          <div style={{
                            ...
                            color: isDefaultLight ? '#9E9E9E' : relevanceScore >= 0.8 ? '#34D399' : relevanceScore >= 0.6 ? '#FBBF24' : '#FB923C',
                          }}>
...
function IdeaGeneratorTab({ isDark, isDefaultLight }: { isDark: boolean; isDefaultLight?: boolean }) {
  const inferenceConfig = useInferenceConfig();
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.25rem' }}>
                      <span style={{
                        ...
                        color: isDefaultLight ? '#9E9E9E' : (overallScore ?? 0) >= 0.7 ? '#34D399' : (overallScore ?? 0) >= 0.4 ? '#FBBF24' : '#F87171',
                      }}>
                        {Math.round((overallScore ?? 0) * 100)}%
                      </span>
                      <StarIcon style={{
                        height: '0.6875rem', width: '0.6875rem',
                        color: isDefaultLight ? '#BDBDBD' : (overallScore ?? 0) >= 0.7 ? '#34D399' : (overallScore ?? 0) >= 0.4 ? '#FBBF24' : (isDark ? 'rgba(156,143,128,0.3)' : 'rgba(0,0,0,0.15)'),
                      }} />
                    </div>
                              ...
                              return (
                                <div key={ri} style={{
                                  ...
                                  {improvements && improvements.length > 0 && (
                                    <div style={{ marginTop: '0.125rem', color: isDefaultLight ? '#9E9E9E' : isDark ? 'rgba(52,211,153,0.7)' : 'rgba(22,101,52,0.6)' }}>
...
function LibraryTab({ isDark, isDefaultLight }: { isDark: boolean; isDefaultLight?: boolean }) {
  ...
  const tagPillStyle = (isActive: boolean): React.CSSProperties => ({
    ...
    color: isActive
      ? (isDark ? 'rgba(232,228,222,0.95)' : 'rgba(28,27,31,0.85)')
                ...
                <div style={{ display: 'flex', gap: '0.375rem', flexWrap: 'wrap', marginTop: '0.375rem' }}>
                  {paper.doi && (
                    <span style={{
                      ...
                      background: isDefaultLight ? '#F0F0F0' : isDark ? 'rgba(139,124,246,0.1)' : 'rgba(139,124,246,0.08)',
                      color: isDefaultLight ? '#9E9E9E' : isDark ? 'rgba(139,124,246,0.8)' : 'rgba(100,80,200,0.7)',
                    }}>
                      DOI: {paper.doi}
                    </span>
                  )}
                  {paper.tags.map((tag) => (
                    <span key={tag} style={{
                      ...
                      fontSize: '0.625rem', padding: '0.0625rem 0.375rem', borderRadius: '9999px',
                      background: isDefaultLight ? '#F0F0F0' : isDark ? 'rgba(52,211,153,0.1)' : 'rgba(52,211,153,0.08)',
                      color: isDefaultLight ? '#9E9E9E' : isDark ? 'rgba(52,211,153,0.8)' : 'rgba(22,101,52,0.7)',
                    }}>
...
export default function ResearchHubPage() {
  ...
  const { isDark, isOled, isSunny } = useIsDark();
    ...
    >
      ...
      <div
        style={{
          ...
          padding: '0.25rem',
          borderRadius: '9999px',
          background: isOled ? 'rgba(18,18,18,0.75)'
            : isDark ? 'rgba(22,21,19,0.65)'
            : 'rgba(0,0,0,0.03)',
          overflowX: 'auto',
            ...
            <button
              ...
              style={{
                ...
                background: isTabActive
                  ? (isOled ? 'rgba(255,255,255,0.95)' : isDark ? 'rgba(44,43,41,0.85)' : 'rgba(0,0,0,0.85)')
                  : 'transparent',
                color: isTabActive
                  ? (isOled ? 'rgba(0,0,0,0.92)' : isDark ? 'rgba(232,228,222,0.95)' : '#FFFFFF')
                  : (isOled ? 'rgba(200,200,200,0.7)' : isDark ? 'rgba(156,143,128,0.7)' : 'rgba(0,0,0,0.45)'),
                boxShadow: 'none',
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          transition={{ duration: 0.2 }}
        >
          {activeTab === 'search' && <PaperSearchTab isDark={isDark} isDefaultLight={isDefaultLight} />}
          {activeTab === 'novelty' && <NoveltyCheckTab isDark={isDark} isDefaultLight={isDefaultLight} />}
          {activeTab === 'review' && <PaperReviewTab isDark={isDark} isDefaultLight={isDefaultLight} />}
          {activeTab === 'citations' && <CitationSearchTab isDark={isDark} isDefaultLight={isDefaultLight} />}
          {activeTab === 'ideas' && <IdeaGeneratorTab isDark={isDark} isDefaultLight={isDefaultLight} />}
          {activeTab === 'library' && <LibraryTab isDark={isDark} isDefaultLight={isDefaultLight} />}
        </motion.div>
      </AnimatePresence>
    </PageShell>
  );
