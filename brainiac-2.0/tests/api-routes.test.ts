...
describe('/api/assistant', () => {
  ...
  it('returns SSE stream for valid input with inferenceConfig', async () => {
    const res = await POST(postRequest('/api/assistant', {
      query: 'What is entropy?',
      context: {},
      inferenceConfig: { mode: 'api', apiProvider: 'openai', openaiModel: 'gpt-4o' },
    }));
    expect(res.status).toBe(200);
    expect(res.headers.get('Content-Type')).toBe('text/event-stream');
  ...
  it('returns 400 when inferenceConfig is missing', async () => {
    const res = await POST(postRequest('/api/assistant', {
      query: 'What is entropy?',
      context: {},
    }));
    ...
    const json = await parseJSON(res);
    expect(json).toHaveProperty('error');
    expect(json.error).toContain('inference');
  });
